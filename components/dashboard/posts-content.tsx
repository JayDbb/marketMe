  ...
  return (
    ...
    >
      <motion.div variants={itemVariants} className="flex flex-col md:flex-row items-start md:items-center justify-between gap-6 mb-8 shrink-0">
        <div>
          <h1 className="text-3xl md:text-4xl font-bold tracking-tighter text-zinc-900 dark:text-white">Posts.</h1>
          <p className="text-zinc-500 dark:text-white/40 mt-2 text-base">Manage and review all your scheduled and published content.</p>
        </div>
      ...
      {/* Posts List */}
      <motion.div variants={itemVariants} className="flex-1 rounded-2xl border border-zinc-200 dark:border-white/10 bg-white dark:bg-white/2 overflow-hidden">
            ...
            <div key={post.id} className="grid grid-cols-12 items-center px-6 py-5 hover:bg-white dark:hover:bg-white/5 border-zinc-200 transition-colors group cursor-pointer">
              <div className="col-span-6 md:col-span-5 pr-4">
                <div className="flex items-start gap-4">
                  <div className="w-10 h-10 rounded-lg bg-white dark:bg-white/5 border-zinc-200 flex items-center justify-center shrink-0 border dark:border-white/10">
                  ...
                  <div>
                    ...
                    {post.status === 'Published' && (
                      <div className="flex items-center gap-4 mt-2">
                        <span className="flex items-center gap-1 text-xs text-zinc-500 dark:text-white/40"><Heart className="w-3 h-3" /> {post.metrics.likes}</span>
                        <span className="flex items-center gap-1 text-xs text-zinc-500 dark:text-white/40"><MessageSquare className="w-3 h-3" /> {post.metrics.comments}</span>
                      </div>
                    )}
                  </div>
                </div>
              ...
              <div className="hidden md:block col-span-2">
                <span className="text-sm text-zinc-500 dark:text-white/70">{post.platform}</span>
              </div>
              ...
              <div className="col-span-3 md:col-span-2">
                <span className={`inline-flex items-center px-2.5 py-1 rounded-md text-xs font-medium border ${
                  ...
                  'bg-white dark:bg-white/5 border-zinc-200 text-zinc-500 dark:text-white/60 dark:border-white/10'
                }`}>
                  {post.status}
                </span>
              </div>
