...
export function UpgradeModal({ open, onOpenChange }: UpgradeModalProps) {
  ...
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="max-w-6xl sm:max-w-6xl bg-[#0c0c18] border-zinc-200 dark:border-white/10 text-zinc-900 dark:text-white p-10 overflow-y-auto max-h-[90vh]">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
          {plans.map((plan) => (
            ...
            >
              ...
              <p className="text-xs text-zinc-500 dark:text-white/40 mb-6 min-h-[32px]">{plan.description}</p>
              
              <div className="mb-6 h-20">
                <span className="text-3xl font-bold text-zinc-900 dark:text-white">${isYearly ? plan.yearlyPrice : plan.monthlyPrice}</span>
                <span className="text-zinc-500 dark:text-white/40 text-sm"> / month</span>
              ...
              <div className="mt-auto">
                <p className="text-[10px] font-bold text-zinc-500 dark:text-white/30 uppercase tracking-wider mb-4">What's included</p>
                <ul className="space-y-4">
                  <li className="flex items-center justify-between text-sm">
                    ...
                    <span className="text-zinc-900 dark:text-white font-medium">{plan.features.workspaces}</span>
                  </li>
                  <li className="flex items-center justify-between text-sm">
                    <span className="text-zinc-500 dark:text-white/60 flex items-center gap-2">
                      <CheckCircle2 className="w-4 h-4 text-zinc-500 dark:text-white/20" />
                    ...
                    <span className="text-zinc-900 dark:text-white font-medium">{plan.features.users}</span>
                  </li>
                  <li className="flex items-center justify-between text-sm">
                    <span className="text-zinc-500 dark:text-white/60 flex items-center gap-2">
                      <CheckCircle2 className="w-4 h-4 text-zinc-500 dark:text-white/20" />
                    ...
                    <span className="text-zinc-900 dark:text-white font-medium">{plan.features.socialProfiles}</span>
                  </li>
                  <li className="flex items-center justify-between text-sm">
                    <span className="text-zinc-500 dark:text-white/60 flex items-center gap-2">
                      <CheckCircle2 className="w-4 h-4 text-zinc-500 dark:text-white/20" />
                    ...
                    <span className="text-zinc-900 dark:text-white font-medium">{plan.features.credits}</span>
                  </li>
                </ul>
              </div>
            </div>
