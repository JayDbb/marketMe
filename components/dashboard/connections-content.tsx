  ...
  return (
    ...
    >
      <div className="flex flex-col lg:flex-row gap-8 items-start">
        ...
        <motion.div variants={itemVariants} className="flex-1 w-full bg-white/4 backdrop-blur-xl border border-white/8 rounded-2xl p-6 shadow-xl relative overflow-hidden">
          <div className="absolute inset-0 bg-linear-to-b from-white/5 to-transparent pointer-events-none" />
          
          <div className="flex items-center justify-between mb-6 relative z-10">
            <h2 className="text-sm font-semibold text-white/50 tracking-wider uppercase">Profiles</h2>
            <div className="flex items-center gap-2">
              <Button className="h-8 bg-blue-500 hover:bg-blue-400 text-white font-bold rounded-lg text-xs shadow-[0_0_15px_rgba(59,130,246,0.2)] border-0">
                Connect profile
              </Button>
            </div>
          </div>
          ...
          <div className="grid grid-cols-2 md:grid-cols-3 gap-4 relative z-10">
            {mockConnections.map((conn, i) => (
              <Card key={i} className="bg-white/5 border-white/10 hover:bg-white/10 transition-colors shadow-none rounded-xl group cursor-pointer">
                <CardContent className="p-4 flex flex-col justify-between h-full">
                  ...
                  <div>
                    <h4 className="font-semibold text-white text-sm truncate">{conn.brand}</h4>
                    <p className="text-[10px] text-white/40 mt-0.5">{conn.time}</p>
                  </div>
                </CardContent>
              </Card>
            ))}
        ...
        <motion.div variants={itemVariants} className="w-full lg:w-[400px] shrink-0 pt-10 lg:pt-20 px-4">
          <h2 className="text-2xl font-bold text-white mb-4">Connect your social profiles</h2>
          ...
          <Button className="h-11 px-8 bg-white text-zinc-950 hover:bg-white/90 font-bold rounded-xl shadow-[0_0_20px_rgba(255,255,255,0.1)] transition-all">
            Connect
          </Button>
        </motion.div>
      </div>
